...
export const data = new SlashCommandBuilder()
  .setName('랭킹')
  .setDescription('서버 멤버의 종합 랭킹을 조회합니다. (서버 기반)');
...
async function buildRows(
  ...
  return Promise.all(
    page.map(async ({ discordUserId, accounts, games, wins, kills, deaths, assists }, i) => {
      ...
      return `${medal} **${memberName}**${titleStr} (${accountsStr})　${games}판 ${wr}% (${wins}승 ${games - wins}패)　KDA ${kda}`;
    }),
  );
}
...
function buildEmbed(rows: string[], page: number, totalPages: number) {
  return new EmbedBuilder()
    ...
    .setColor(0xffd700)
    .setFooter({
      text: `서버 기반 · 판수 → 승률 → KDA 순 정렬 · ${page + 1}/${totalPages} 페이지`,
    })
    .setTimestamp();
}
