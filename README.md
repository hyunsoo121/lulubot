...
| 명령어 | 설명 |
|--------|------|
| `/계정등록 닉네임#태그` | 본인의 라이엇 계정 연결 + 백그라운드 전체 스캔 |
| `/멤버등록 @유저 닉네임#태그` | 관리자가 다른 유저 계정 등록 (관리자 전용) |
| `/유저계정정보` | 연결된 계정 확인 |
| `/계정삭제` | 본인 데이터 전체 삭제 |
...
| 명령어 | 설명 |
|--------|------|
| `/전적갱신 [@멤버]` | 본인 전적 갱신 (3분 쿨다운). 다른 멤버 지정은 관리자 전용 |
| `/유저전적 [@유저]` | 서버 내 커스텀 게임 누적 통계 + 보유 칭호 |
| `/유저전체전적 [@유저]` | 서버 구분 없이 모든 커스텀 게임 기준 커리어 |
| `/최근경기 [@유저]` | 마지막 내전 결과 상세 |
| `/랭킹` | 서버 멤버 종합 랭킹, 판수 → 승률 → KDA 순 (기본: 서버 기반, 서버기반/시즌 필터 옵션) |
| `/종합랭킹` | 서버 등록 계정의 전체 게임 기준 종합 랭킹 |
| `/라인랭킹 라인:` | 라인별 특화 지표 랭킹, 표 형태로 표시 (기본: 서버 기반, 서버기반/시즌 필터 옵션) |
| `/모스트챔피언 [@유저]` | 해당 유저의 모스트 챔피언 (전체 게임 기준) |
| `/챔피언랭킹 챔피언:` | 서버 등록 계정의 챔피언별 랭킹, 판수 → 승률 → KDA 순 (전체 게임 기준, autocomplete, 서버기반/시즌 필터 옵션) |
| `/전적비교 유저1: 유저2:` | 두 유저 전적 나란히 비교 (서버기반/시즌 필터 옵션) |
| `/칭호` | 서버 칭호 보유자 목록 (서버 기반 고정) |
| `/칭호순위` | 특정 칭호 전체 순위 (서버 기반 고정, autocomplete) |
| `/듀오전적` | 듀오 조합별 전적, 판수 → 승률 → KDA 순 (기본: 서버 기반, 서버기반/시즌 필터 옵션) |
| `/진영선택 팀장1: 팀장2:` | 이미 나눈 두 팀 중 블루/레드를 랜덤으로 배정 |
...
### 필터 옵션 (서버기반 / 시즌)
`/랭킹`, `/라인랭킹`, `/듀오전적`, `/챔피언랭킹`, `/전적비교`가 지원합니다.
- **서버기반**: 매치 참가자 10명 중 8명 이상이 이 서버 등록 계정인 매치만 포함. `/랭킹`·`/라인랭킹`·`/듀오전적`은 기본값 true(서버 기반), `/챔피언랭킹`·`/전적비교`는 기본값 false(전체 게임 기준)
- **시작일 / 종료일**: `YYYY-MM-DD` 형식으로 날짜 범위 지정 (시즌별 조회)
- **칭호/칭호순위**는 옵션 없이 항상 서버 기반으로 계산됩니다.
