...
| `/랭킹` | 서버 멤버 종합 랭킹, 판수 → 승률 → KDA 순 (서버 기반) |
| `/종합랭킹` | 서버 등록 계정의 전체 게임 기준 종합 랭킹 |
| `/라인랭킹 라인:` | 라인별 특화 지표 랭킹 (서버 기반) |
| `/모스트챔피언 [@유저]` | 해당 유저의 모스트 챔피언 (전체 게임 기준) |
| `/챔피언랭킹 챔피언:` | 서버 등록 계정의 챔피언별 랭킹, 판수 → 승률 → KDA 순 (전체 게임 기준, autocomplete) |
| `/전적비교 유저1: 유저2:` | 두 유저 전적 나란히 비교 (서버 기반) |
| `/칭호` | 서버 칭호 보유자 목록 (서버 기반) |
| `/칭호순위` | 특정 칭호 전체 순위 (서버 기반, autocomplete) |
| `/듀오전적` | 듀오 조합별 전적, 판수 → 승률 → KDA 순 (서버 기반) |
| `/밴픽률` | 챔피언 밴률/픽률 TOP 10 (서버 기반). 밴 데이터는 이 기능 도입 이후 새로 갱신된 매치부터만 집계됨 |
| `/진영선택 팀장1: 팀장2:` | 이미 나눈 두 팀 중 블루/레드를 랜덤으로 배정 |

**서버 기반**: 매치 참가자 10명 중 8명 이상이 이 서버 등록 계정인 매치만 포함 (옵션 없이 고정 동작). `/챔피언랭킹`만 전체 게임 기준으로 고정.
