...
### `player_match_stat`에 인덱스가 하나도 없어 모든 통계 쿼리가 풀 테이블 스캔
**증상**: 지금은(매치 수가 적어서) 체감 안 되지만, 전체 마이그레이션 히스토리를 확인해보니 이 프로젝트의 핵심 테이블인 `player_match_stat`에 PK(`id`) 말고는 인덱스가 **0개**. 그런데 `titleService.ts`/`stats.ts`/`matchFilter.ts`/`matchScan.ts`의 거의 모든 쿼리가 `where: { matchId: { in: [...] } }` 또는 `where: { lolAccountId: { in: [...] } }`로 이 테이블을 필터링함  
**원인**: `@@index` 없이 스키마를 짜서 Postgres가 두 컬럼 다 인덱스 없이 매번 시퀀셜 스캔. 매치 데이터는 계속 쌓이기만 하는 테이블이라, **매치 수와 응답 시간이 비례해서 같이 늘어나는 구조** — 특정 서버 하나가 아니라 테이블이 전체 서버 공유라서 한 서버가 매치를 많이 쌓을수록 다른 서버 커맨드까지 같이 느려짐. 새 매치를 저장하는 `saveMatch`도 `matchId` 기준 조회를 하기 때문에 읽기뿐 아니라 쓰기(스캔) 경로도 같이 느려짐  
**해결**: `player_match_stat(match_id)`, `player_match_stat(lol_account_id)`, `player_match_stat(champion_id)`, `champion_ban(match_id)`에 인덱스 추가(`prisma/migrations/20260922091219_add_player_match_stat_indexes`). **매치 1만 건 기준으로 인덱스 없이는 조건에 맞는 행을 찾으려면 최악의 경우 1만 행을 전부 훑어야 하는데(O(N)), 인덱스가 있으면 B-tree 탐색으로 로그 스케일(O(log N), 1만 건 기준 약 14단)로 줄어듦** — 매치 수가 늘어날수록 격차가 기하급수적으로 벌어짐. 데이터가 아직 적을 때 미리 추가해서, 나중에 테이블이 커진 뒤 인덱스 생성 자체가 부하가 되는 상황을 피함

---

### Discord API 멤버 조회가 랭킹류 커맨드 6곳에서 전부 "목록 순회하며 1명씩" 방식
**증상**: `/랭킹`, `/종합랭킹`, `/칭호`, `/칭호순위`, `/챔피언랭킹`, `/라인랭킹` — 표시 이름이 필요한 랭킹/칭호 커맨드 6개 전부, 목록의 각 항목마다 `guild.members.fetch(id)`를 개별 호출(DB의 N+1과 같은 모양의 문제, 대상이 Discord REST API라는 점만 다름)  
**원인**: 여러 명을 한 번에 가져오는 `guild.members.fetch({ user: [...] })`는 REST가 아니라 게이트웨이 방식이라 `GuildMembers` 특권 인텐트가 필요한데, 이 봇은 `GatewayIntentBits.Guilds`만 켜져 있어 지금은 배치로 못 묶음(3번 항목에서 확인)  
**현재 상태**: 페이지네이션 재조회 문제(위 "페이지네이션 버튼 클릭마다..." 항목)는 고쳐서 **커맨드 실행당 1번씩만** 호출되지만, 그 1번이 여전히 "인원수만큼 개별 호출"임 — 예를 들어 등록 인원 30명 서버의 `/랭킹`은 여전히 매 실행마다 최대 30건의 개별 REST 호출이 나감  
**해결 방향(미착수)**: `GuildMembers` 인텐트를 켜고(Developer Portal + 코드) 배치 fetch로 전환하면 30건 → 1건으로 줄일 수 있음. 인텐트 추가는 봇이 100개 서버를 넘기면 Discord 검증이 필요해지는 등 별도 검토가 필요해 이번엔 보류
